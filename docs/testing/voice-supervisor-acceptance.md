...
版本：0.1.0-alpha.24。基线：alpha.23 / DSH 0.1.5-rc.2。

## alpha.24 修复（真人测试反馈）

- 听过的汇报不再滞留在列表：交付确认后条目直接移除；同时保留真正没听完的中断可重听。
- 汇报被打断的概率问题定位到本地 VAD：扬声器外放漏进麦克风，在播报刚开始时被判成“用户在说话”，于是取消了 AI 自己的汇报。现在起音判定要求连续 4 帧（160ms）持续人声，并在播放开始后 600ms 的回声收敛窗口内不再由本地单独取消，交给供应端 VAD 决定。
- 取消与播放排空确认在网络上会交叉：取消先到会清掉流映射，使已经播完的汇报永远无法确认交付。现在取消会短暂保留该流的交付凭据（15 秒，且有单调序号校验），让已发出的确认仍然生效。
- 安排任务改为两步：总管先念出候选任务或“哪个项目＋哪个 Agent”，问一句“是这个吗”，只有用户明确回答“对／可以／就这个”才绑定；含糊或否定回答一律不绑定，相近任务不会被自动认成同一个。
- 播报口语化：新增 `toSpokenText`，去掉 markdown、状态标签、括号、编号、路径、链接、哈希，蛇形命名改写成词，长列表改为计数；总管指导与内置 Skill 同步加上“像打电话一样说话”的规则。
...
- 总管新增 `confirm_voice_task`：绑定已有任务或创建新任务前必须由用户口头确认。
...
- `pnpm test`：51 个测试文件、288 项测试通过；`pnpm build` 与 `pnpm verify` 通过。
- 真实麦克风下的打断手感、两步确认的语音识别命中率、口语化听感仍需安装 alpha.24 后人工验收。
本次开发使用隔离工作树，不读取凭据明文，不修改 DSH 源码。

## 自动化证据

- `pnpm test`：288 项测试（51 个文件）通过。
...
| 本地安装与桌面启动 | PASS | desktop profile link 指向当前仓库；DSH Desktop 2.0.13 已启动，alpha.24 启动日志无插件错误 |
| 拨号球单击即拨、长按/右键才选人 | PARTIAL | voice-launcher 测试覆盖单击与右键；真实长按手势与 Mac 触控板待验收 |
| 听过的汇报从列表移除，未听完的中断仍保留 | PASS | voice-inbox（交付即移除）+ report-delivery（未排空仍 interrupted）测试 |
| 汇报不会因扬声器回声被自己打断 | PARTIAL | local-vad 起音门槛与 client-transcript 回声收敛窗口测试；真实房间回声需人工复测 |
| 取消与播放确认交叉时不丢交付 | PASS | voice-audio-flow「drain 跨过 cancel」测试 |
| 任务对象必须由用户口头确认才绑定 | PASS | voice-supervisor 两步确认 + voice-intent-confirmation 测试 |
| 播报文本已口语化，不念标点括号路径 | PASS | spoken-text 测试覆盖 markdown、括号、路径、链接、哈希、蛇形命名与长列表 |
| 最新桌面实际菜单 | NOT TESTED | Mac 已锁定；Desktop 禁止普通浏览器访问，HTTP 返回 403，未修改此设置 |

## 必须保留的边界

- 汇报被打断的概率问题已定位到本地 VAD 误判扬声器回声，并按此修正；但房间回声、外放音量和麦克风阵列差异很大，仍需在不同设备上复测确认。
- 口语化处理只改写书写形式，不改数字、结论和状态词。它与模型自己的表达仍可能不完全一致，真实听感需要人工确认。
- 两步确认依赖语音识别把“对／可以／就这个”准确转成文字；识别不到时会重复询问，不会误绑定，但可能多问一轮。
...
- alpha.23 基线已推送至 `feat/voice-supervisor` 与 `feat/dsh-0.1.5-compat`；alpha.24 的最终提交见对应版本标签。
- 本地安装位置：`/Users/bofeng/Development/WorkSpace/my/AI/dsh-realtime-voice`；
  原 desktop profile link 保持不变，通过 fast-forward 同步构建产物。
- 同步后在实际 link 目标再次运行 `pnpm test`（288/288）和 `pnpm verify`，通过。
...
2. 新建一位叫「运维」的总管并接通，口头说“你以后负责日志清理”，确认它记住范围；
   再新建「写作」并接通，确认它不知道运维那件事。
3. 通话中说“换成写作来接”，确认它自报新身份，并重新确认要做什么。
4. 让总管安排已有任务时，确认它先把任务名念出来问“是这个吗”；
   回答“不对，是另一个”应继续询问，回答“对，就这个”才真正绑定。
5. 新建任务时确认它先说明“在哪个项目、用哪个 Agent”，得到同意后才创建。
6. 选择安全测试项目和 Agent，新建测试任务；说“创建 hello.txt，写 hello voice supervisor”。
7. 工作中补充“不要修改其他文件”，确认进入同一任务。
8. 挂断后等完成，接听回拨，不讲话，确认汇报完整且**没有中途自己停掉**；再测试主动插话。
9. 听完一条汇报后，确认它从列表里消失，而不是留在那里。
10. 汇报里带 markdown、括号、路径或长列表时，确认听到的是口语化短句，没有念出标点、括号和绝对路径。
11. 请求“汇报啊”，对象不明时应询问；不应修改或合并文件。
12. 设计一个需要问题/审批的安全任务，确认回拨和口头回答。
13. 点击已读后取消，条目应保留；确认已读仅移除汇报，不删除会话。
14. 响铃设置为 10 秒，跨多个轮询观察；点击稍后立即停铃。
15. 多条报告分别接听/重听，确认旧播放回执不会误清新报告。
16. 设置语音总管 Skill 为空、有效路径、无效路径分别测试；与执行 Agent 指导区分。
