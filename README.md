...
当前版本：`0.1.0-alpha.19`，目标 DSH：`0.1.5-rc.2`。这是独立语音总管测试版，真实麦克风与桌面交互验收状态见 [验收记录](docs/testing/voice-supervisor-acceptance.md)。
...
回拨的「已读」先确认、请求成功才移除；「稍后」显示状态并保存。回拨记录使用 DSH storage domain 持久化；重启恢复的旧审批要求回原任务处理，不能直接批准。汇报只有供应端完成且浏览器确认对应音频序号播放排空后才标记已交付，中断保留重听。旧客户端缺少播放序号确认时不推断“已听完”。
