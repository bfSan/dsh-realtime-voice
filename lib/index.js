...
var VoiceSupervisor = class {
	...
	constructor(callId, directory, actions) {
		this.callId = callId;
...
async function resolveSupervisorGuidance(options, runtime) {
	...
	if (skill && !looksLikeGuidancePath(skill) && !options.cwd) return {
		body: DEFAULT_SUPERVISOR_GUIDANCE,
		source: skill,
		status: "error",
		error: "请先选择项目以加载语音总管 Skill，或使用文件绝对路径。"
	};
...
var VoiceConnection = class {
	...
	* echo filtering. Capable clients keep forwarding near-end speech/pre-roll. */
	...
	progressGate;
	progressCoalescer;
	constructor(ctx, socket, request, config, onClosed, runtime = new VoiceRuntime(), inbox = void 0, guidanceRuntime = {}, directory) {
		this.ctx = ctx;
		this.socket = socket;
	...
	async receive(raw, isBinary) {
		...
		if (isSupervisorHello(parsed)) {
			...
			await this.start({
				...
				target: { sessionId: SUPERVISOR_LEASE_TARGET }
			});
...
function apply(ctx, config) {
	...
	const upgradeProxy = (request, socket, head) => {
		...
		proxyServer.handleUpgrade(request, socket, head, (websocket) => {
			let connection;
			connection = new VoiceConnection(ctx, websocket, request, activeConfig, () => connections.delete(connection), voiceRuntime, inbox, guidanceRuntime, directory);
