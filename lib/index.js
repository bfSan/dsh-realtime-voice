...
var ReportDelivery = class {
	...
	playbackDrained(attemptId) {
		this.advance(attemptId, "drained");
	}
	interrupt(attemptId) {
		const attempt = this.attempts.get(attemptId);
		if (attempt !== void 0 && attempt.state !== "completed") attempt.state = "interrupted";
	}
	state(reportId) {
	...
	*/
	deliverInboxEntries(entryIds) {
		...
		this.progressCoalescer.settle({
			id: attemptId,
			text: `[COMPLETE] 用户挂断后完成的任务结果，共 ${ordered.length} 条：\n${summary}\n${single ? "这是用户刚从回拨列表里选中的任务。请用一两句话汇报结论，不要重复提交这个任务。" : "这些是用户刚从回拨列表里按顺序选中的任务。请按顺序逐条用一两句话汇报结论，不要重复提交这些任务。"}`,
			body: summary,
			kind: "complete",
			group: attemptId
		});
...
var VoiceInbox = class {
	...
	*/
	markHandoffDelivered(handoffId) {
		...
		if (entry.delivered) return void 0;
		entry.delivered = true;
		this.onChange?.();
		return { ...entry };
	}
