...
var VoiceConnection = class {
	...
	* echo filtering. Capable clients keep forwarding near-end speech/pre-roll. */
	...
	progressGate;
	progressCoalescer;
	constructor(ctx, socket, request, config, onClosed, runtime = new VoiceRuntime(), inbox = void 0, guidanceRuntime = {}) {
		this.ctx = ctx;
		this.socket = socket;
	...
	*/
	...
	async start(hello) {
		...
		const instructions = buildVoiceInstructions(status, this.continuity, { stylePrompt: this.config.stylePrompt });
...
var VoiceInbox = class {
	...
	*/
	observeNeedsInput(input, interaction) {
		...
		while (this.entries.length > 20) this.entries.shift();
		return { ...entry };
	}
	/**
	...
	*/
	...
	completeTurn(sessionId, status, summary) {
...
function apply(ctx, config) {
	...
	const upgradeProxy = (request, socket, head) => {
		...
		proxyServer.handleUpgrade(request, socket, head, (websocket) => {
			let connection;
			connection = new VoiceConnection(ctx, websocket, request, activeConfig, () => connections.delete(connection), voiceRuntime, inbox, guidanceRuntime);
