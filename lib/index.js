...
*/
function registerDefaultHandoffSkill(ctx) {
	try {
		const skills = ctx.skills;
		if (skills === void 0) return void 0;
		return skills.register(defaultHandoffSkill());
	} catch (error) {
		ctx.logger?.warn?.(`[realtime-voice] built-in handoff skill could not be registered: ${error instanceof Error ? error.message : String(error)}`);
		return;
	}
}
//#endregion
...
*/
async function resolveHandoffGuidance(ctx, config, options) {
	const skillName = config.handoffSkill.trim();
	const inline = config.handoffInstructions.trim();
	const skillBody = skillName === "" ? void 0 : await loadSkillBody(ctx, skillName, options);
...
async function loadSkillBody(ctx, skillName, options) {
	try {
		const skills = ctx.skills;
		if (skills === void 0) {
		...
		ctx.logger?.warn?.(`[realtime-voice] failed to load handoff skill "${skillName}": ${error instanceof Error ? error.message : String(error)}`);
		return;
	}
}
//#endregion
...
var VoiceConnection = class {
	...
	* echo filtering. Capable clients keep forwarding near-end speech/pre-roll. */
	...
	progressGate;
	progressCoalescer;
	constructor(ctx, socket, request, config, onClosed, runtime = new VoiceRuntime(), inbox = void 0) {
		this.ctx = ctx;
		this.socket = socket;
	...
	*/
	...
	async start(hello) {
		...
		this.functionBridge = new DshFunctionBridge(coordinator, this.continuity.functionReceipts, {
			...
			resolveGuidance: async () => await resolveHandoffGuidance(this.ctx, this.config, status.cwd === void 0 ? {} : { cwd: status.cwd }),
...
var DirectControlConnection = class {
	...
	helloTimer;
	heartbeatTimer;
	constructor(ctx, socket, request, config, onClosed, runtime, temporaryKeys = new TemporaryKeyService(), inbox = void 0) {
		this.ctx = ctx;
		this.socket = socket;
		this.request = request;
		this.config = config;
...
function apply(ctx, config) {
	...
	ctx.effect(() => registerDefaultHandoffSkill(ctx) ?? (() => {}), "realtime-voice: built-in handoff skill");
	...
	const upgradeDirect = (request, socket, head) => {
		...
		directServer.handleUpgrade(request, socket, head, (websocket) => {
			let connection;
			connection = new DirectControlConnection(ctx, websocket, request, activeConfig, () => connections.delete(connection), voiceRuntime, void 0, inbox);
			connections.add(connection);
		});
	};
