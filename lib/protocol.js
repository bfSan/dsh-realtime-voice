...
function isVoiceClientControl(value) {
	...
	if (message.type === "voice.cancel-response") return message.source === void 0 || message.source === "local-vad" || message.source === "user";
	if (message.type === "voice.inbox-deliver") return Array.isArray(message.entryIds) && message.entryIds.length > 0 && message.entryIds.length <= 20 && message.entryIds.every((id) => typeof id === "string" && id.length > 0 && id.length <= 128);
	if (message.type === "voice.playback-drained") return typeof message.streamId === "number" && Number.isSafeInteger(message.streamId) && message.streamId >= 0 && (message.lastSequence === void 0 || typeof message.lastSequence === "number" && Number.isSafeInteger(message.lastSequence) && message.lastSequence >= 0);
