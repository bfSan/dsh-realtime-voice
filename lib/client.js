window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		const VOICE_WEB_CLIENT_VERSION = "0.1.0-alpha.17";
...
`;
		...
		*/
		...
		function VoiceOverlay({ useVoice, useVoiceModelSettings, useSessions, end, toggleMute, cancelResponse, answerApproval, answerQuestion, answerInbox, answerInboxOne, snoozeInbox, toggleInboxSelection, selectAllInbox, clearInboxSelection, dismissInbox, openSession }) {
			...
			const newWaitingIds = waiting.map((entry) => entry.id).filter((id) => !rungRef.current.has(id) && !snoozed.includes(id));
			(0, react.useEffect)(() => {
			...
			const beginDrag = (event) => {
				...
				if (!collapsed && event.target.closest(INTERACTIVE_SELECTOR) !== null) return;
		...
		function VoiceSettingsCard({ useVoiceModelSettings, selectModel, selectTurnDetection, selectVoice, setVadThreshold, setSilenceDuration, setMaxHistoryTurns, setSpeechEmotion, setStylePrompt, selectProgressReporting, setProgressMinInterval, setProgressQuietTask, setHandoffSkill, setHandoffInstructions, setRingDuration, saveApiKey }) {
